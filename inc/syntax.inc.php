...
function newConnect()
{
	$conn = mysqli_connect("localhost", "root", "5032cuidados", "rieles");
	return $conn;
}
...
function month_toString($mes_nro, $mayus = 0, $year = 0)
{
	...
	switch (intval($mes_nro)) {
		case 1:
			$mes = ($mayus == 0 ? 'Enero' : 'ENERO');
			break;
		case 2:
			$mes = ($mayus == 0 ? 'Febrero' : 'FEBRERO');
			break;
		case 3:
			$mes = ($mayus == 0 ? 'Marzo' : 'MARZO');
			break;
		case 4:
			$mes = ($mayus == 0 ? 'Abril' : 'ABRIL');
			break;
		case 5:
			$mes = ($mayus == 0 ? 'Mayo' : 'MAYO');
			break;
		case 6:
			$mes = ($mayus == 0 ? 'Junio' : 'JUNIO');
			break;
		case 7:
			$mes = ($mayus == 0 ? 'Julio' : 'JULIO');
			break;
		case 8:
			$mes = ($mayus == 0 ? 'Agosto' : 'AGOSTO');
			break;
		case 9:
			$mes = ($mayus == 0 ? 'Septiembre' : 'SEPTIEMBRE');
			break;
		case 10:
			$mes = ($mayus == 0 ? 'Octubre' : 'OCTUBRE');
			break;
		case 11:
			$mes = ($mayus == 0 ? 'Noviembre' : 'NOVIEMBRE');
			break;
		case 12:
			$mes = ($mayus == 0 ? 'Diciembre' : 'DICIEMBRE');
			break;
	}
